...
export class QuestionnaireItemChoiceComponent
  ...
  implements ControlValueAccessor, OnDestroy
{
    ...
    if (this.parentGroup) {
      ...
      if (this.qformControl === undefined) {
        throw new Error(
          "linkId '" +
            this.item.linkId +
            "' not found in parentGroup controls. Ensure linkId element exists in '" +
  ...
  private filterOptionsCoding(
    criteria: string,
    maxlist: number
  ): Observable<fhirclient.FHIR.Coding[]> {
    return of(
      this.answerOption
        ?.filter((option) =>
