...
@Component({
  ...
  export class QuestionnaireItemDecimalComponent extends QuestionnaireItemBase implements ControlValueAccessor, AfterViewInit {

    //@Output() 
    private valueChange; //= new EventEmitter();
    value;

    constructor(qresponseService: QuestionnaireResponseService, private injector: Injector) {
...
*/        
        ...
        if (ngControl) {
            ...
            if (this.formControl.item?.extension) {
                const calculatedExpression = this.item.extension.find(e=> e.url === 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-calculatedExpression')?.valueExpression;
            
                if (calculatedExpression) {
                    var root = this.formControl.root as QuestionnaireForm;
                    root.addCalculatedExpression(this.formControl, calculatedExpression.expression);
                }
            }    
        } else {
