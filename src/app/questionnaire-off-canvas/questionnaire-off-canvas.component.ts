...
export class QuestionnaireOffCanvasComponent implements OnDestroy {
  ...
  saveQResponse(qresponse: QuestionnaireResponse) {
    this.subscriptions.push(
      this.responseService.create(qresponse).subscribe(
        (y) => {
          console.log(y);
  ...
  validateQResponse(qresponse: QuestionnaireResponse) {
    this.subscriptions.push(
      this.responseService.validate(qresponse).subscribe(
        (y) => console.log(y),
        (e) => console.log(e)
      )
    );
  ...
  extractResponseInstance() {
    this.subscriptions.push(
      this.responseService.extractInstance(this.responseService.id).subscribe(
        (y) => {
          console.log(y);
          this.bundle = y;
        },
  ...
  postObservation() {
    ...
    this.subscriptions.push(
      this.fhirService.create(resource).subscribe(
        (y) => {
          console.log(y);
        },
        (e) => console.log(e)
