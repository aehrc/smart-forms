...
The `buildForm()` function is used to build the form. It takes in [BuildFormsParams](/docs/api/smart-forms-renderer/interfaces/BuildFormParams) as arguments.
...
| additionalContext       | `Record<string, any>`                                                                                                                 | Additional key-value pairs of SDC variables and values to feed into the renderer's FhirPathContext. Likely used for passing in data from a pre-population module. Example: `{ 'ObsBodyHeight': <Bundle of height observations> }`.                                            | Optional  |
| preserveNavigationState | boolean                                                                                                                               | Whether to preserve the current navigation state (e.g. current page in paged forms, current tab in tabbed forms) when rebuilding the form.<br/>This is useful when you want to perform re-population or other updates without losing the user's current position in the form. | Optional  |
| rendererConfigOptions   | [`RendererConfig`](/docs/api/smart-forms-renderer/interfaces/RendererConfig)                                                          | Optional renderer styling and behavioural configurations to have fine-grained control over the styling and behaviour of the renderer.                                                                                                                                         | Optional  |
| qItemOverrideComponents | [`Record<string, ComponentType<QItemOverrideComponentProps>>`](/docs/api/smart-forms-renderer/interfaces/QItemOverrideComponentProps) | Key-value pairs of React component overrides for specific Questionnaire Items via linkId.<br/>Example: `{ 'linkId123': MyCustomComponent }`                                                                                                                                   | Optional  |
| sdcUiOverrideComponents | [`Record<string, ComponentType<SdcUiOverrideComponentProps>>`](/docs/api/smart-forms-renderer/interfaces/SdcUiOverrideComponentProps) | Key-value pairs of React component overrides for SDC UI Controls, as defined in: https://hl7.org/fhir/extensions/ValueSet-questionnaire-item-control.html.<br/>Example: `{ 'flyover': MyCustomUIComponent }`                                                                  | Optional  |
...
| questionnaire         | FHIR R4.Questionnaire                                    | Questionnaire to be rendered                                                                                                                                                                                                       | Required  |
| questionnaireResponse | FHIR R4.QuestionnaireResponse                            | Pre-populated/draft/loaded QuestionnaireResponse to be rendered                                                                                                                                                                    | Optional  |
| additionalVariables   | `Record<string, object>`                                 | Additional key-value pairs of SDC variables and values to feed into the renderer's FhirPathContext. Likely used for passing in data from a pre-population module. Example: `{ 'ObsBodyHeight': <Bundle of height observations> }`. | Optional  |
| terminologyServerUrl  | string                                                   | Terminology server url to fetch terminology. If not provided, the default terminology server will be used.                                                                                                                         | Optional  |
| fhirClient            | [FHIRClient](https://github.com/smart-on-fhir/client-js) | FHIRClient to perform further FHIR calls (This helps you set `fhirClient` and `patient`, `user`, `encounter` launch contexts)                                                                                                      | Optional  |
| readOnly              | boolean                                                  | Applies read-only mode to all items in the form view                                                                                                                                                                               | Optional  |
