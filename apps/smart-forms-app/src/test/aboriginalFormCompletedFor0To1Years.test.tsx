...
test('Completed questionnaire for 0 years', async () => {
  ...
  await inputInteger(container, 'Age', 0);
  ...
  await checkRadioOption(container, 'Child has a birth certificate', 'Yes');
  ...
  await inputDecimal(container, 'Gestation at birth', 4.5);
  await inputDecimal(container, 'Birth weight', 3.5);
  ...
  await inputText(
    container,
    "Is there anything that you are worried about with your child's development?",
  ...
  await inputText(
    container,
    'What childcare/early learning centre/kindergarten does your child go to? How often?',
    'Other concerns about learning and development'
  );
  await inputText(
    container,
    'How often does your child miss childcare/early learning centre/kindergarten?',
    'Other concerns about learning and development'
  );
  await inputText(
    container,
  ...
  await checkRadioOption(
    ...
    "Is there anything that you are worried about with your child's feeding/eating?",
    'Yes'
  );
  await inputText(childEatingConcernsContainer, 'Details', 'Feeding concerns');
  await inputText(container, 'Breastfeeding history', 'Breastfeeding history');
  await inputText(container, 'Bottle feeding history', 'Bottle feeding history');
  await inputText(
    container,
    'Document conversation about age-appropriate healthy eating, which could include: current diet; transition to solids; dietary recommendations re fruit and vegetable intake, iron-rich foods, avoiding sugary drinks',
    'food'
  );
  ...
  await checkCheckboxOption(
    container,
    'Cognition, fine motor and self care',
    'Does not explore objects with hand and mouth'
  );
  await checkCheckboxOption(
    container,
    'Cognition, fine motor and self care',
  ...
  await checkCheckboxOption(
    container,
    'Gross motor',
    'Not holding head with good control in supported sitting'
  );
  await checkCheckboxOption(container, 'Red flags at any age', 'Strong parental concerns');
  await checkCheckboxOption(container, 'Red flags at any age', 'Significant loss of skills');
  await checkCheckboxOption(
    container,
  ...
  await checkCheckboxOption(container, 'Red flags at any age', 'Lack of, or limited eye contact');
  await checkCheckboxOption(
    container,
    'Red flags at any age',
  ...
  const redReflexContainer = await findByLinkIdOrLabel(container, 'Red reflex (up to six months)');
  await inputText(redReflexContainer, 'Left eye', 'Left eye');
  await inputText(redReflexContainer, 'Right eye', 'Right eye');
  ...
  await inputDecimal(container, 'obs-lengthheight-newresult', 50.5);
  ...
  await inputText(
    container,
    'Newborn examination (if indicated, infant aged <=6 weeks)',
    'Newborn examination'
  );
  ...
  await inputText(
    container,
    'Patient priorities and goals: What does the parent/carer say are the important things that have come out of this health check?',
  ...
  await inputText(container, 'Parent/carer actions', 'Parent/carer actions');
  await checkRadioOption(
    container,
