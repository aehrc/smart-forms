...
test('Completed questionnaire for 49 years', async () => {
  ...
  await checkRadioOption(learningContainer, 'Are you studying?', 'Yes');
  await inputText(container, 'Details', 'Studying details');
  ...
  await checkRadioOption(
    ...
    'Is there anything that you are worried about in relation to your sexual health?',
    'Yes'
  );
  await inputText(sexualHealthConcernsContainer, 'Details', 'Sexual health concerns');
  const cervicalScreeningContainer = await findByLinkIdOrLabel(container, 'Cervical screening');
  await checkCheckboxOption(cervicalScreeningContainer, 'Cervical screening status', 'Declined');
  await checkCheckboxOption(cervicalScreeningContainer, 'Cervical screening status', 'Offered');
  await checkCheckboxOption(
    cervicalScreeningContainer,
    'Cervical screening status',
    'Not required'
  );
  await checkCheckboxOption(cervicalScreeningContainer, 'Cervical screening status', 'Up to date');
  await inputDate(cervicalScreeningContainer, 'Next due', '01/01/2024');
  await inputText(cervicalScreeningContainer, 'Details', 'Cervical screening details');
  await inputText(
    container,
    'Consider discussing as relevant to age/sex/gender: contraception; menstruation; sexually transmitted infection symptoms and screening; blood-borne virus screening; continence; menopause; erectile dysfunction',
