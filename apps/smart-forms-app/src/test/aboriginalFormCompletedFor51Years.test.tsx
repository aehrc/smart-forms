...
test('Completed questionnaire for 51 years', async () => {
  ...
  await checkRadioOption(yourCarerContainer, 'Do you have a carer?', 'Yes');
  await inputText(yourCarerContainer, 'Details', 'Mother');
  ...
  const enduringPowerOfAttorneyContainer = await findByLinkIdOrLabel(
    container,
    'Enduring Power of Attorney'
  );
  await checkRadioOption(enduringPowerOfAttorneyContainer, 'Enduring Power of Attorney', 'Yes');
  await inputText(
    enduringPowerOfAttorneyContainer,
    'Discussed today',
    'Enduring Power of Attorney details'
  );
  const advanceHealthDirectiveContainer = await findByLinkIdOrLabel(
    container,
    'Advance Health Directive'
  );
  await checkRadioOption(advanceHealthDirectiveContainer, 'Advance Health Directive', 'Yes');
  await inputText(
    advanceHealthDirectiveContainer,
    'Discussed today',
  ...
  await inputText(container, 'Who/details', 'Doctor');
  ...
  await inputText(
    container,
    'Consider conversation about social connection, which could include questions about sports/hobbies/clubs/other activities',
    'Sports/hobbies/clubs/other activities'
  );
  await inputText(
    container,
    'Health priorities, actions and follow-up',
  ...
  await checkRadioOption(container, 'Are you working?', 'Yes');
  await inputText(
    container,
  ...
  const personalConcernsMemoryOrThinkingContainer = await findByLinkIdOrLabel(
    container,
    'Personal concerns about your memory or thinking'
  );
  await checkRadioOption(
    personalConcernsMemoryOrThinkingContainer,
    'Do you have any worries about your memory or thinking?',
    'Yes'
  );
  await inputText(
    personalConcernsMemoryOrThinkingContainer,
    'Details',
    'Memory or thinking concerns'
  );
  const familyConcernsMemoryOrThinkingContainer = await findByLinkIdOrLabel(
    container,
    'Family concerns about your memory or thinking'
  );
  await checkRadioOption(
    familyConcernsMemoryOrThinkingContainer,
    'Does anyone in your family have any worries about your memory or thinking?',
    'Yes'
  );
  await inputText(
    familyConcernsMemoryOrThinkingContainer,
    'Details',
    'Family memory or thinking concerns'
  );
  await inputText(
    container,
    'If any concerns are raised and/or high risk for cognitive impairment identified, follow up with cognitive screening (eg clock test, GPCOG, KICA-Cog, MMSE)',
    'Cognitive screening'
  );
  await inputText(
    container,
  ...
  //Chronic disease associated with ageing
  await selectTab(container, 'Chronic disease associated with ageing');
  await inputText(
    container,
    'For older people with chronic disease and/or conditions associated with ageing, consider assessing: Osteoporosis risk; Falls risk; Balance, coordination and mobility; Pain; Nutrition; Regularity of chronic disease management and review',
    'Assessment of chronic disease associated with ageing'
  );
  await inputText(
    container,
    'Health priorities, actions and follow-up',
  ...
  const cervicalScreeningStatusContainer = await findByLinkIdOrLabel(
    container,
    'Cervical screening (age <=74 years)'
  );
  await checkRadioOption(
    cervicalScreeningStatusContainer,
    'Cervical screening status',
    'Discussed today'
  );
  await inputDate(cervicalScreeningStatusContainer, 'Cervical screening next due', '01/01/2024');
  await inputText(
    cervicalScreeningStatusContainer,
    'Cervical screening details',
    'Cervical screening details'
  );
  const breastCancerScreeningStatusContainer = await findByLinkIdOrLabel(
    container,
    'Breast screening (age <=74 years)'
  );
  await checkRadioOption(
    breastCancerScreeningStatusContainer,
    'Breast screening status',
    'Discussed today'
  );
  await inputDate(breastCancerScreeningStatusContainer, 'Breast screening next due', '01/01/2024');
  await inputText(
    breastCancerScreeningStatusContainer,
    'Breast screening details',
    'Breast screening details'
  );
  const bowelCancerScreeningStatusContainer = await findByLinkIdOrLabel(
    container,
    'Bowel cancer screening (age <=74 years)'
  );
  await checkRadioOption(
    bowelCancerScreeningStatusContainer,
    'Bowel cancer screening status',
    'Discussed today'
  );
  await inputDate(
    bowelCancerScreeningStatusContainer,
    'Bowel cancer screening next due',
    '01/01/2024'
  );
  await inputText(
    bowelCancerScreeningStatusContainer,
    'Bowel cancer screening details',
    'Bowel cancer screening details'
  );
  await inputText(
    container,
  ...
  const unintendedWeightLossContainer = await findByLinkIdOrLabel(
    container,
    'Unintended weight loss'
  );
  await checkRadioOption(
    unintendedWeightLossContainer,
    'Have you lost weight without trying to in the last three months?',
    'Yes'
  );
  await inputText(unintendedWeightLossContainer, 'Details', 'Unintended weight loss');
  await checkRadioOption(
    unintendedWeightLossContainer,
    'Have you lost weight without trying to in the last three months?',
    'Yes'
  ...
  await checkRadioOption(physicalActivityConcernsContainer, 'Do you have any worries about physical activity?', 'Yes');
  await inputText(physicalActivityConcernsContainer, 'Details', 'Physical activity concerns');
  await inputText(
    container,
    'Document conversation about recommendations re physical activity',
  ...
  const sexualHealthConcernsContainer = await findByLinkIdOrLabel(
    container,
    'Sexual health concerns'
  );
  await checkRadioOption(
    sexualHealthConcernsContainer,
    'Consider asking when appropriate: Is there anything that you are worried about in relation to your sexual health?',
    'Yes'
  );
  await inputText(sexualHealthConcernsContainer, 'Details', 'Sexual health concerns');
  const continenceIssuesContainer = await findByLinkIdOrLabel(container, 'Continence issues');
  await checkRadioOption(
    continenceIssuesContainer,
    'Do you have problems with urine leaking?',
    'Yes'
  );
  await inputText(continenceIssuesContainer, 'Details', 'Continence issues');
  await inputText(
    container,
    'Consider discussing as relevant to age/sex/gender: menopause; erectile dysfunction; prostatic symptoms',
    'menopause; erectile dysfunction; prostatic symptoms'
  );
  await inputText(
    container,
    'Health priorities, actions and follow-up',
  ...
  await checkCheckboxOption(trachomaCheckContainer, 'Right eye', 'Corneal scar');
  await inputText(
    container,
    'Health priorities, actions and follow-up',
    'Health priorities, actions and follow-up'
  ...
  await checkRadioOption(container, 'Dentures?', 'Yes');
  ...
  await inputDecimal(container, 'obs-waistcircumference-newresult', 20.5);
  ...
  await inputText(container, 'Latest available result', 'Not available');
  await checkRadioOption(
    container,
  ...
  await checkRadioOption(
    container,
    'Do you need to manually enter a new CVD risk result instead of repopulating?',
    'Yes'
  );
  await inputInteger(container, 'Risk result score', 4);
  await checkRadioOption(container, 'Assessed risk category', 'High Risk');
  await inputText(
    container,
    'Health priorities, actions and follow-up',
    'Health priorities, actions and follow-up'
  ...
  await inputText(
    container,
    'Full blood count, HbA1c or blood glucose level, serum lipids, Kidney function including eGFR, Liver function tests, ACR',
    'Blood tests'
  );
  await inputText(
    container,
  ...
  expect(await getInputText(tabContainer, 'Work')).toBe('Health priorities, actions and follow-up');
  ...
  expect(await getInputText(tabContainer, 'Physical activity')).toBe(
    'Health priorities, actions and follow-up'
  );
  ...
  expect(await getInputText(tabContainer, 'Investigations')).toBe(
    'Health priorities, actions and follow-up'
  );
  ...
  //await checkCheckboxOption(container, 'Brief intervention: advice and information provided during health check', 'Other');
  await inputText(
    container,
