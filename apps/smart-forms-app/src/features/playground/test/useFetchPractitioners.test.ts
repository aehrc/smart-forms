...
import useFetchPractitioners from '../hooks/useFetchPractitioners';
...
// Mock globals
jest.mock('../../../globals', () => ({
  NUM_OF_PRACTITIONERS_TO_FETCH_PLAYGROUND: 100
}));

describe('useFetchPractitioners', () => {
  ...
  it('returns empty practitioners array when no data is fetched', async () => {
    ...
    const { result } = renderHook(() => useFetchPractitioners('https://example.com/fhir'), {
  ...
  it('returns practitioners when valid bundle is fetched', async () => {
    ...
    expect(result.current.practitioners).toEqual([mockPractitioner]);
    expect(result.current.fetchStatus).toBe('success');
    expect(result.current.fetchError).toBeNull();
  });
  ...
  it('returns empty array for non-practitioner bundle', async () => {
    const mockBundle: Bundle = {
      resourceType: 'Bundle',
      type: 'searchset',
      entry: [
  ...
  it('calls fetchFhirResources with correct parameters', async () => {
    ...
    renderHook(() => useFetchPractitioners('https://example.com/fhir'), {
      wrapper: createWrapper
    });
  ...
  it('filters out null or undefined resources from entries', async () => {
    ...
    const mockBundle: Bundle<Practitioner> = {
      ...
      entry: [
        { resource: mockPractitioner },
        { resource: undefined },
        { resource: null as unknown as Practitioner },
        { resource: mockPractitioner }
      ]
    };
    ...
    expect(result.current.practitioners).toEqual([mockPractitioner, mockPractitioner]);
  });
